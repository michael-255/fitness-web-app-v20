...
const chartLabel = 'Best Resistance'
const chartData: Ref<{
  labels: any[]
  datasets: any[]
}> = ref({
...
async function recalculateChart() {
  try {
    ...
    if (!exerciseInputs?.includes(ExerciseInput.RESISTANCE)) return
    ...
    const dataItems = timeRestrictedRecords.map((record: AnyDBRecord) =>
      Math.max(...record.resistance)
    )

    chartData.value = {
      labels: chartLabels,
      datasets: [getSingleChartDataset(dataItems, 'negative', 'negative')],
    }

    isVisible.value = true
  } catch (error) {
    log.error('Error loading exercise best resistance chart', error)
  }
}
</script>
