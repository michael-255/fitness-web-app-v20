...
const chartLabel = 'Best Calories Burned'
const chartData: Ref<{
  labels: any[]
  datasets: any[]
}> = ref({
...
async function recalculateChart() {
  try {
    ...
    if (!exerciseInputs?.includes(ExerciseInput.CALORIES)) return
    ...
    log.error('Error loading exercise best calories chart', error)
  }
}
</script>
