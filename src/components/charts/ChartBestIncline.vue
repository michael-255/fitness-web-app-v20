...
async function recalculateChart() {
  try {
    ...
    if (!exerciseInputs?.includes(ExerciseInput.INCLINE)) return
    ...
    log.error('Error loading exercise best incline chart', error)
  }
}
</script>
