...
const chartLabel = 'Best Duration (minutes)'
const chartData: Ref<{
  labels: any[]
  datasets: any[]
}> = ref({
...
async function recalculateChart() {
  try {
    ...
    if (!exerciseInputs?.includes(ExerciseInput.DURATION)) return
    ...
    log.error('Error loading exercise best duration chart', error)
  }
}
</script>
